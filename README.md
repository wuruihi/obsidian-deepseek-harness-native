...
- **服务自动管理**：启动命令留空即**自动探测本机 DSH**（PATH 上的 `dsh`、`~/dsh`、`~/deepseek-harness` …），打开面板时若本地 DSH 未运行则自动拉起；内置「一键安装」克隆并安装 DSH。
...
- 或手动安装 DSH 后，把插件设置里的「启动命令」**留空**（推荐：自动探测），或填写你自己的启动方式（插件会以 vault 为工作目录启动它）。
...
| 启动命令 | 留空 = 自动探测本机 DSH 安装（推荐，跨机器可用）；也可手填，`{port}` 自动替换；需以 vault 为 cwd |
